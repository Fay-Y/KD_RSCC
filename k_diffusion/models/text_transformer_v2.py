...
def downscale_pos(pos):
    pos = rearrange(pos, "... n_w,w_s, e -> ... (n_w) e", nh=2, nw=2)
...
def _apply_rotary_emb_inplace(x, theta, conj):
    ...
    cos, sin = torch.cos(theta), torch.sin(theta)
    print('cos sin',cos.size(),sin.size())
    sin = -sin if conj else sin
    y1 = x1_ * cos - x2_ * sin
    y2 = x2_ * cos + x1_ * sin
    x1.copy_(y1)
...
def window_text(window_size, x):
    # x shape  (batch_size, seq_len, d_head)
    *b, seq_len, c = x.shape
    x = torch.reshape(
        x,
        (*b, seq_len // window_size, window_size, c),  #  (batch_size,n_heads, num_windows, window_size, d_head)
    )
    # x = torch.permute(
    #     x,
    #     (*range(len(b)), -3, -1, -2),  
...
def shifted_window_text(window_size, window_shift, x):
    # x 转换为窗口序列
    x = torch.roll(x, shifts=window_shift, dims=-2)  
    windows = window_text(window_size, x)  
    
    return windows

def shifted_unwindow_text(window_shift, x):
    # Reverse the rolling and windowing operation for text sequences
    *b, seq_len, window_size, d_head = x.shape
    x = x.view(*b, seq_len * window_size, d_head)
    x = torch.roll(x, shifts=(-window_shift,), dims=-2)  # Reverse the shift operation
    return x
...
def make_shifted_window_masks_text(n_windows, window_size, shift, device=None):
    seq_coords = torch.arange(window_size, device=device)

    # 计算窗口内每个位置与其他位置的距离
    q_coords, k_coords = torch.meshgrid(seq_coords, seq_coords, indexing="ij")

    # 生成单个窗口内的掩码
    single_window_mask = torch.abs(q_coords - k_coords) <= shift

    # 扩展到所有窗口
    masks = single_window_mask.unsqueeze(0).expand(n_windows, -1, -1)
...
def apply_window_attention_text(window_size, window_shift, q, k, v, scale=None):
    # prep windows and masks
    q_windows = shifted_window_text(window_size, window_shift, q)
    k_windows = shifted_window_text(window_size, window_shift, k)
    v_windows = shifted_window_text(window_size, window_shift, v)
    print('q_window',q_windows.size())
    b, heads,n_w,w_s,d_head = q_windows.shape
    mask = make_shifted_window_masks_text(n_w,window_size, window_shift, device=q.device)
    # q_seqs = torch.reshape(q_windows, (b, heads, n_w*w_s, d_head))
    # k_seqs = torch.reshape(k_windows, (b, heads, n_w*w_s, d_head))
    # v_seqs = torch.reshape(v_windows, (b, heads,n_w*w_s, d_head))
    q_seqs = q_windows
    k_seqs = k_windows
    v_seqs = v_windows
    # print('q_seq',q_seqs.size())
    ...
    # unwindow
    qkv = torch.reshape(qkv, (b, heads, n_w,w_s, d_head))
    return shifted_unwindow_text(window_shift, qkv)
# Transformer layers
...
class ShiftedWindowSelfAttentionBlock(nn.Module):
    def __init__(self, d_model, d_head, cond_features, window_size, window_shift, dropout=0.0):
        ...
        self.scale = nn.Parameter(torch.full([self.n_heads], 10.0))
        # self.pos_emb = AxialRoPE(d_head // 2, self.n_heads)
        # self.pos_emb = RoPE(d_head=self.d_head // 2, n_heads=self.n_heads)
        self.dropout = nn.Dropout(dropout)
        self.out_proj = apply_wd(zero_init(Linear(d_model, d_model, bias=False)))
    ...
    def forward(self, x, pos, cond):
        skip = x
        print("x in shifted window",x.size())
        x = self.norm(x, cond)
        print("x norm in shifted window",x.size())
        qkv = self.qkv_proj(x)#(bs,seqlen,dmodel*3)
        print('qkv',qkv.size())
        # q, k, v = rearrange(qkv, "n h w (t nh e) -> t n nh h w e", t=3, e=self.d_head)
        q, k, v = rearrange(qkv, "n l (t nh e) -> t n nh l e", t=3, e=self.d_head) #(bs,12,seqlen,dmodel)
        # q, k = scale_for_cosine_sim(q, k, self.scale[:, None, None, None], 1e-6)
        # theta = self.pos_emb(pos).movedim(-2, -4)
        # q = apply_rotary_emb_(q, theta)
        # k = apply_rotary_emb_(k, theta)
        ...
        x = apply_window_attention_text(self.window_size, self.window_shift, q, k, v, scale=1.0)
...
class TokenMerge(nn.Module):
    def __init__(self, in_features, out_features, window_size):
        super().__init__()
        # self.h = patch_size[0]
        # self.w = patch_size[1]
        self.proj = apply_wd(Linear(in_features*window_size, out_features, bias=False))

    def forward(self, x):
        # x = rearrange(x, "... (h nh) (w nw) e -> ... h w (nh nw e)", nh=self.h, nw=self.w)
        x = rearrange(x, "... (n_w window_size) (h d_head) -> ... n_w window_size h d_head", window_size=self.window_size)
        return self.proj(x)
...
class TextTransformerDenoiserModelV2(nn.Module):
    def __init__(self, levels, mapping, in_channels, out_channels,vocab_size,  mapping_cond_dim=0):
        super().__init__()
        ...
        # self.word_embedding = CLIPTextModel.from_pretrained(version).cuda()
        ...
        self.lm_head = nn.Sequential(
                        ...
                        nn.LeakyReLU(),  
                        nn.Dropout(p=0.1),  
                        nn.Linear(in_channels, vocab_size))
    ...
    def forward(self, x, sigma, aug_cond=None, class_cond=None, mapping_cond=None):
        ...
        pos = make_axial_pos(x.shape[-2], x.shape[-1], device=x.device).view(x.shape[-2], x.shape[-1], 2)
        print('pos',pos.shape)
        print('x',x.shape)
        # print('cond',mapping_cond.shape)
        # Mapping network
        ...
        for down_level, merge in zip(self.down_levels, self.merges):
            ...
            skips.append(x)
            poses.append(pos)
            print('x in texttransformer',x.size())
            x = merge(x)
