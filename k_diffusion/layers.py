...
class Denoiser(nn.Module):
    ...
    def loss(self, input, noise, sigma, **kwargs):
        w = self.inner_model.get_embeds(input)
        c_skip, c_out, c_in = [utils.append_dims(x, w.ndim) for x in self.get_scalings(sigma)]
        c_weight = self.weighting(sigma)
        noise = torch.randn_like(w)
        noised_input = w + noise * utils.append_dims(sigma, w.ndim)
        # print('noised_input',noised_input.size())
        # print('c_in',c_in.size())
        model_output = self.inner_model(noised_input * c_in, sigma, **kwargs)
        
        target = (w - c_skip * noised_input) / c_out
        get_logits = self.inner_model.get_logits
        decoder_nll = self.token_discrete_loss(noised_input, get_logits, input)

        if self.scales == 1:
            # return decoder_nll*c_weight
            # # print('c  weight',c_weight)
            return ((model_output - target) ** 2).flatten(1).mean(1) * c_weight+decoder_nll *c_weight
        ...
        # print('loss1',(sq_error * f_weight).flatten(1).mean(1) * c_weight)
        # print("loss classifier",decoder_nll)
        return (sq_error * f_weight).flatten(1).mean(1) * c_weight + decoder_nll
