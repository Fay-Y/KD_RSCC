...
from CCdataloader import CCDataset
import os
...
def main():
    writer = SummaryWriter('loss_tensorboard')
    ...
    p.add_argument('--demo-every', type=int, default=1500,
    ...
    elif dataset_config['type'] == 'levircc':    
        data_folder = dataset_config['data_folder'] 
        split = dataset_config.get('split', 'train')  # 默认使用训练集
        train_set = CCDataset(data_folder=data_folder, split=split)
    else:
    ...
    # print('test show,',image_key,num_classes,cond_dropout_rate,class_key)
    ...
    #     x = torch.zeros([1,size[0], model_config['input_channels']], device=device)
    #     sigma = torch.ones([1], device=device)
    #     extra_args = {}
    #     extra_args['mapping_cond'] = torch.zeros([1,,1024], device=device)
    ...
    def demo():
        if accelerator.is_main_process:
            tqdm.write('Generating text based on image conditions...')
         # 获取批次和模型输入
        n_per_proc = math.ceil(args.sample_n / accelerator.num_processes)
        
        # 从 testdataset 中随机抽取一个样本
        data_folder = dataset_config['data_folder'] 
        test_set = CCDataset(data_folder=data_folder, split='test')
        test_dl =  data.DataLoader(test_set, n_per_proc, shuffle=True, drop_last=True)
        test_iter = iter(test_dl)  
        test_sample = next(test_iter)
        testcond1,testcond2,test_caps,test_name = test_sample
        testcond1,testcond2 = inner_model.img_process(testcond1.cuda(),testcond2.cuda())
        condition_image = testcond1-testcond2
        # condition_image = condition_image.contiguous()

       
        # x = torch.randn([accelerator.num_processes, n_per_proc,size[0], model_config['input_channels']], generator=demo_gen).to(device)
        # dist.broadcast(x, 0)
        # x = x[accelerator.process_index] * sigma_max
        # condition_image = condition_image[:n_per_proc].to(device)  
        # dist.broadcast(condition_image, 0)  
        
        x = torch.randn([ n_per_proc,size[0], model_config['input_channels']], generator=demo_gen).to(device)
        dist.broadcast(x, 0)
        x = x * sigma_max
        ...
        #     condition_image = condition_image[accelerator.process_index]


        model_fn, extra_args = model_ema, {}
        # print('demo cond',condition_image.size())
        extra_args['mapping_cond'] = condition_image  # 将条件图像传入模型


        sigmas = K.sampling.get_sigmas_karras(50, sigma_min, sigma_max, rho=7., device=device)
        generated_text = K.sampling.sample_dpmpp_2m_sde(
            model_fn,
            x = x,
            sigmas=sigmas,
        ...
        # generated_text = accelerator.gather(generated_text)[:args.sample_n]
        if accelerator.is_main_process:
            # print('generated_text',generated_text.size())
            x_t = generated_text.clone().detach()
            logits = inner_model.get_logits(x_t)  # bsz, seqlen, vocab
            cands = torch.topk(logits, k=1, dim=-1)
            # sample = cands.indices
            with open(os.path.join(dataset_config['data_folder'],'vocab.json'), 'r') as f:
                    vocab = json.load(f)
            ...
            for idx, (name, text) in enumerate(zip(test_name, word_lst_e2e)):
                filename = os.path.join('demo_result', f'step_{step}_{name}_result.txt')
                with open(filename, 'w') as f:
                    f.write(f"{name}:\n{text}\n")

            if use_wandb:
                for name, text in zip(test_name, word_lst_e2e):
                    wandb.log({f'Generated_Text_{name}': text}, step=step)
    # @torch.no_grad()
    # @K.utils.eval_mode(model_ema)
    # def evaluate():
    ...
    def save():
        ...
        if accelerator.is_main_process:
            tqdm.write(f'Saving to {checkpoint_path}...')
        ...
        accelerator.save(obj, checkpoint_path)
        if accelerator.is_main_process:
            state_obj = {'latest_checkpoint': checkpoint_path}
    ...
    try:
        while True:
            for batch in tqdm(train_dl, smoothing=0.1, disable=not accelerator.is_main_process):
                cond1,cond2,caps,filename = batch
            
            # for cond1,cond2,caps,filename in train_dl:
                ...
                with accelerator.accumulate(model):
                    ...
                    # caps = inner_model.get_embeds(caps)
                    inner_model = inner_model.cuda()
                    cond1,cond2 = inner_model.img_process(cond1.cuda(),cond2.cuda())
                    # print("caps",caps.size())
                    extra_args['mapping_cond'] = cond1-cond2
                    # extra_args['cond1'] = cond1
                    # extra_args['cond2'] = cond2
                    
                    
                    noise = None #torch.randn_like(caps)
                    with K.utils.enable_stratified_accelerate(accelerator, disable=args.gns):
                        sigma = sample_density([caps.shape[0]], device=device)
                        
                    # print("sizie check !!!!!!!!!!!",caps.size())
                    with K.models.checkpointing(args.checkpointing):
                        # aug_cond = None
                        
                        losses = model.loss(caps, noise,sigma, **extra_args)
                    ...
                    if args.gns:
                        ...
                        gns_stats.update(sq_norm_small_batch, sq_norm_large_batch, caps.shape[0], caps.shape[0] * accelerator.num_processes)
